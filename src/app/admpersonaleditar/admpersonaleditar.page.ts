...
@Component({
  selector: 'app-admpersonaleditar',
  templateUrl: './admpersonaleditar.page.html',
  styleUrls: ['./admpersonaleditar.page.scss'],
})
export class AdmPersonalEditarPage {
  handleRefresh(event) {
    setTimeout(() => {
      this.ngOnInit();
      ...
      if (this.per_id === undefined) {
        const response = await axios({
          ...
          if (response?.status == 201) {
            this.alertGuardado(response.data.per_id, 'El alumno con matricula ' + response.data.per_matricula + ' ha sido registrada');
          }
        }).catch((error) => {
        ...
        const response = await axios({
          ...
          if (response?.status == 200) {
            this.alertGuardado(response.data.per_id, 'El alumno con matricula ' + response.data.per_id + ' ha sido actualizado');
          }
        }).catch((error) => {
          if (error?.response?.status == 422) {
            this.alertGuardado(personal.per_id, error?.response?.data[0]?.message, "Error");
